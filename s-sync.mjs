...
function normalizeEquipmentId(id) {
  ...
  if (/^KVDM2\d{3}$/.test(s)) return s;
  if (/^ASL3K\d{3}$/.test(s)) return s;
  if (/^RFX\d{3}$/.test(s)) return s;
...
console.log("Target family counts in page:", counts);
  ...
  // const asl3kRows = rowsRaw.filter(r => (r.equipment_id || "").includes("ASL3K"));
  // console.log(("ASL3K rowsRaw:", asl3kRows.length, asl3kRows.slice(0, 10).map(r => r.equipment_id)));
